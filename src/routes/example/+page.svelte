<script lang="ts">
	console.info('example: +page.svelte');
...
<section>
		...
		<Dialog.Portal>
			...
			<Dialog.Content class="fixed top-0 bg-mauve-1">
				...
				<Dialog.Root defaultOpen={false}>
					...
					<Dialog.Portal>
						<Dialog.Overlay
							class="fixed top-0 w-full h-full bg-mauve-12/50"
						/>
						<Dialog.Content class="fixed top-0 bg-mauve-1">
	...
	>
		<Tabs.List
			ariaLabel="trigger list"
			class="flex p-2 space-x-2 overflow-x-auto "
		>
	...
	<Accordion.Root
		type="multiple"
		defaultValue="2"
		class="m-2 border border-mauve-6"
	>
		{#each content as { value, disabled }}
			<Accordion.Item
				{value}
